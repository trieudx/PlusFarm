...
#include <spi_flash.h>
#include "../../../framework/lwip/lwip/sdk/include/c_types.h"
#include "../../../platform/core/newlib/xtensa-lx106-elf/include/string.h"
...
rboot_get_config(void)
{
  rboot_config conf;
  spi_flash_read(BOOT_CONFIG_SECTOR * SECTOR_SIZE, (uint32*)&conf,
                 sizeof(rboot_config));
  return conf;
}
...
rboot_set_config(rboot_config *conf)
{
  uint8 *buffer;
  ...
  spi_flash_read(BOOT_CONFIG_SECTOR * SECTOR_SIZE, (uint32*)((void*)buffer),
  SECTOR_SIZE);
  ...
  spi_flash_write(BOOT_CONFIG_SECTOR * SECTOR_SIZE, (uint32*)((void*)buffer),
  SECTOR_SIZE);
...
bool ICACHE_FLASH_ATTR
rboot_set_current_rom(uint8 rom)
{
  rboot_config conf;
  conf = rboot_get_config();
  if (rom >= conf.count)
