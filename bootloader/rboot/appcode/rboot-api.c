...
#if USE_OS
#include "log.h"
#define RBOOT_DEBUG(f_, ...) LOG_PRINTF((f_), __VA_ARGS__)
#else
...
{
  rboot_config conf;
  sdk_spi_flash_read(BOOT_CONFIG_SECTOR * SECTOR_SIZE, (uint32_t*)&conf,
                     sizeof(rboot_config));
  return conf;
}
...
bool rboot_set_config(rboot_config *conf)
{
  ...
  sdk_spi_flash_read(BOOT_CONFIG_SECTOR * SECTOR_SIZE, (uint32_t*)((void*)buffer),
                     SECTOR_SIZE);
  ...
  vPortEnterCritical();
#endif
  sdk_spi_flash_write(BOOT_CONFIG_SECTOR * SECTOR_SIZE, (uint32_t*)((void*)buffer),
                  SECTOR_SIZE);
#if USE_OS
  vPortExitCritical();
#endif
                                         ...
                                         uint8_t *data, uint16_t len)
{
  ...
  while (lastsect > status->last_sector_erased)
  {
    ...
    sdk_spi_flash_erase_sector(status->last_sector_erased);
  }

  // write current chunk
  //os_printf("write addr: 0x%08x, len: 0x%04x\r\n", status->start_addr, len);
  if (sdk_spi_flash_write(status->start_addr, (uint32_t *)((void*)buffer), len) == SPI_FLASH_RESULT_OK)
  {
    ret = true;
    status->start_addr += len;
  }
...
{
  if (sdk_system_rtc_mem_read(RBOOT_RTC_ADDR, rtc, sizeof(rboot_rtc_data)))
  {
...
bool rboot_set_rtc_data(rboot_rtc_data *rtc)
{
  ...
  return sdk_system_rtc_mem_write(RBOOT_RTC_ADDR, rtc, sizeof(rboot_rtc_data));
}
...
#define ROM_MAGIC_NEW 0xea
    ...
    bool rboot_verify_image(uint32_t initial_offset, uint32_t *image_length,
                            const char **error_message)
    {
      ...
      if (sdk_spi_flash_read(offset, (uint32_t *)&image_header,
      ...
      if (image_header.magic != ROM_MAGIC_OLD && image_header.magic
          != ROM_MAGIC_NEW)
      {
      ...
      while (remaining_sections > 0 && offset < end_limit)
      {
        ...
        if (sdk_spi_flash_read(offset, (uint32_t *)&header,
        ...
        RBOOT_DEBUG(
            "Found section @ 0x%08x (abs 0x%08x) length %d load 0x%08x\n",
        ...
        if (is_new_header)
        {
          ...
          sdk_spi_flash_read(offset, (uint32_t *)&image_header,
                             sizeof(image_header_t));
          offset += sizeof(image_header_t);
