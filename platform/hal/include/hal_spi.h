...
extern "C"
{
   ...
   */
  bool spi_init(uint8_t bus, spi_mode_t mode, uint32_t freq_divider, bool msb,
                spi_endianness_t endianness, bool minimal_pins);
  /**
   * \brief Initalize SPI bus
   * spi_init() wrapper.
   * Example:
  ...
  {
    return spi_init(bus, s->mode, s->freq_divider, s->msb, s->endianness,
                    s->minimal_pins);
  }
  /**
  ...
  {
    return APB_CLK_FREQ / (FIELD2VAL(SPI_CLOCK_DIV_PRE, SPI(bus).CLOCK) + 1)
           / (FIELD2VAL(SPI_CLOCK_COUNT_NUM, SPI(bus).CLOCK) + 1);
  }
  ...
  inline spi_endianness_t spi_get_endianness(uint8_t bus)
  {
    return
        SPI(bus).USER0 & (SPI_USER0_WR_BYTE_ORDER | SPI_USER0_RD_BYTE_ORDER) ?
            SPI_BIG_ENDIAN : SPI_LITTLE_ENDIAN;
  }
   ...
   */
  size_t spi_transfer(uint8_t bus, const void *out_data, void *in_data,
                      size_t len, spi_word_size_t word_size);
  ...
  static inline void spi_set_command(uint8_t bus, uint8_t bits, uint16_t data)
  {
    ...
    SPI(bus).USER2 = SET_FIELD(SPI(bus).USER2, SPI_USER2_COMMAND_BITLEN,
                               --bits);
    SPI(bus).USER2 = SET_FIELD(SPI(bus).USER2, SPI_USER2_COMMAND_VALUE,
                               command);
  }
  ...
  static inline void spi_set_dummy_bits(uint8_t bus, uint8_t bits, bool pos)
  {
    ...
    if (pos)
    {
      SPI(bus).USER0 |= SPI_USER0_MISO;
    } // Dummy bit will be between Dout and Din data if set
    SPI(bus).USER0 |= SPI_USER0_DUMMY; //enable dummy bits
    SPI(bus).USER1 = SET_FIELD(SPI(bus).USER1, SPI_USER1_DUMMY_CYCLELEN,
                               --bits);
  }
