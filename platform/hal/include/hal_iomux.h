...
extern "C"
{
  ...
  inline static void iomux_set_direction_flags(uint8_t iomux_num,
                                               uint32_t dir_flags)
  {
  ...
  inline static void iomux_set_pullup_flags(uint8_t iomux_num,
                                            uint32_t pullup_flags)
  {
    uint32_t mask = IOMUX_PIN_PULLUP | IOMUX_PIN_PULLDOWN
                    | IOMUX_PIN_PULLUP_SLEEP | IOMUX_PIN_PULLDOWN_SLEEP;
    uint32_t prev = IOMUX.PIN[iomux_num] & ~mask;
   ...
   * same as before.
   */
  inline static void iomux_set_gpio_function(uint8_t gpio_number,
                                             bool output_enable)
  {
    ...
    iomux_set_direction_flags(iomux_num,
                              output_enable ? IOMUX_PIN_OUTPUT_ENABLE : 0);
  }

  inline static void gpio_set_iomux_function(const uint8_t gpio_num,
                                             uint32_t func)
  {
