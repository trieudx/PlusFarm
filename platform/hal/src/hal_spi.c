              ...
              spi_endianness_t endianness, bool minimal_pins)
{
  ...
  SPI(bus).USER0 = SPI_USER0_MOSI
      | SPI_USER0_CLOCK_IN_EDGE | SPI_USER0_DUPLEX
...
void spi_set_frequency_div(uint8_t bus, uint32_t divider)
{
  ...
  {
    IOMUX.CONF &= ~(
        bus == 0 ? IOMUX_CONF_SPI0_CLOCK_EQU_SYS_CLOCK :
...
static void _spi_buf_prepare(uint8_t bus, size_t len, spi_endianness_t e,
                             spi_word_size_t word_size)
{
  ...
  {
    data[i] =
        word_size == SPI_16BIT ? _swap_words(data[i]) : _swap_bytes(data[i]);
  }
}

static void _spi_buf_transfer(uint8_t bus, const void *out_data, void *in_data,
                              size_t len, spi_endianness_t e,
                              spi_word_size_t word_size)
{
  _wait(bus);
                    ...
                    size_t len, spi_word_size_t word_size)
{
  ...
  for (size_t i = 0; i < blocks; i++)
  {
    ...
    _spi_buf_transfer(bus, (const uint8_t *)out_data + offset,
                      in_data ? (uint8_t *)in_data + offset : NULL, buf_size, e,
                      word_size);
    if (blocks)
  ...
  {
    _spi_buf_transfer(
        bus, (const uint8_t *)out_data + blocks * _SPI_BUF_SIZE,
        in_data ? (uint8_t *)in_data + blocks * _SPI_BUF_SIZE : NULL, tail, e,
        word_size);
  }
