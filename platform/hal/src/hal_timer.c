...
const static uint32_t IROM _TIMER_FREQS[] = { _FREQ_DIV1, _FREQ_DIV16,
_FREQ_DIV256 };
...
{
  const uint32_t dp_bit =
      (frc == FRC1) ? DPORT_INT_ENABLE_FRC1 : DPORT_INT_ENABLE_FRC2;
  const uint32_t int_mask = BIT(
      (frc == FRC1) ? INUM_TIMER_FRC1 : INUM_TIMER_FRC2);
  if (enable)
  {
...
uint32_t timer_freq_to_count(const timer_frc_t frc, const uint32_t freq,
                             const timer_clkdiv_t div)
{
...
uint32_t timer_time_to_count(const timer_frc_t frc, uint32_t us,
                             const timer_clkdiv_t div)
{
  if (div < TIMER_CLKDIV_1 || div > TIMER_CLKDIV_256)
    return 0; /* invalid divider */
  ...
  if (div != TIMER_CLKDIV_256) /* timer tick in MHz */
  {
    ...
    const uint32_t counts_per_us = (
        (div == TIMER_CLKDIV_1) ? _FREQ_DIV1 : _FREQ_DIV16)
                                   / 1000 / 1000;
