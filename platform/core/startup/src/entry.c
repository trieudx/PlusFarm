...
static void IRAM set_spi0_divisor(uint32_t divisor)
{
  ...
  else
  {
    ...
    clkdiv = VAL2FIELD(
        SPI_CTRL0_CLOCK_NUM,
        cycle_len) | VAL2FIELD(SPI_CTRL0_CLOCK_HIGH, half_cycle_len)
...
void IRAM sdk_user_start(void)
{
  ...
  sysparam_addr = flash_size
      - (4 + DEFAULT_SYSPARAM_SECTORS) * sdk_flashchip.sector_size;
  status = sysparam_init(sysparam_addr, flash_size);
  if (status == SYSPARAM_NOTFOUND)
  {
    status = sysparam_create_area(sysparam_addr, DEFAULT_SYSPARAM_SECTORS,
    false);
...
static __attribute__((noinline)) void user_start_phase2(void)
{
  ...
  _Static_assert(
      configCPU_CLOCK_HZ == 80000000 || configCPU_CLOCK_HZ == 160000000,
      "FreeRTOSConfig must define initial clock speed as either 80MHz or 160MHz");
...
static __attribute__((noinline)) void dump_flash_config_sectors(
