...
static sysparam_status_t _write_and_verify(uint32_t addr, const void *data,
                                           size_t data_size)
{
...
static inline sysparam_status_t _write_region_header(uint32_t addr,
                                                     uint32_t other,
                                                     bool active)
{
...
static sysparam_status_t init_write_context(struct sysparam_context *ctx)
{
  ...
  CHECK_FLASH_OP(
      spiflash_read(ctx->addr, (void*) &ctx->entry, ENTRY_HEADER_SIZE));
  return SYSPARAM_OK;
}
 ...
 *                   to the end.
 */
static sysparam_status_t _find_entry(struct sysparam_context *ctx,
                                     uint16_t match_id, bool find_value)
{
  uint16_t id;
  ...
  while (true)
  {
    ...
    else
    {
      ...
      if (next_addr > _sysparam_info.cur_base + _sysparam_info.region_size)
      {
        ...
        debug(
            1,
            "Encountered entry with invalid length (0x%04x) @ 0x%08x (region end is 0x%08x).  Truncating entries.",
            ctx->entry.len, ctx->addr, _sysparam_info.end_addr);
...
static inline sysparam_status_t _read_payload(struct sysparam_context *ctx,
                                              uint8_t *buffer,
                                              size_t buffer_size)
{
...
static inline sysparam_status_t _compare_payload(struct sysparam_context *ctx,
                                                 uint8_t *value, size_t size)
{
  debug(3, "compare payload (%d) @ 0x%08x", size, ctx->addr);
  if (ctx->entry.len != size)
    return SYSPARAM_NOTFOUND;
                                   ...
                                   const char *key, uint16_t key_len)
{
  ...
  debug(3, "key match @ 0x%08x (idflags = 0x%04x)", ctx->addr,
        ctx->entry.idflags);
...
static inline sysparam_status_t _find_value(struct sysparam_context *ctx,
                                            uint16_t id_field)
{
...
static inline sysparam_status_t _write_entry(uint32_t addr, uint16_t id,
 ...
 */
static sysparam_status_t _compact_params(struct sysparam_context *ctx,
                                         int *key_id)
{
  ...
  debug(1,
        "compacting region (current size %d, expect to recover %d%s bytes)...",
  ...
  while (true)
  {
    ...
    status = _write_entry(addr, current_key_id, (uint8_t *)iter.key,
                          iter.key_len);
    if (status < 0)
      break;
    ...
    status = _write_entry(addr, current_key_id | ENTRY_FLAG_VALUE | binary_flag,
  ...
  debug(1, "done compacting (current size %d)",
        _sysparam_info.end_addr - _sysparam_info.cur_base);
...
sysparam_status_t sysparam_init(uint32_t base_addr, uint32_t top_addr)
{
  ...
  if (header1.magic == SYSPARAM_MAGIC)
  {
    ...
    if ((header0.flags_size & REGION_FLAG_SECOND) == (header1.flags_size
        & REGION_FLAG_SECOND))
    {
      // Hmm.. they both say they're the same region.  That can't be right...
      debug(
          1,
          "Found region headers @ 0x%08x and 0x%08x, but both claim to be the same region.",
          addr0, addr1);
  ...
  else
  {
    ...
    if (header0.flags_size & REGION_FLAG_ACTIVE)
    {
      ...
      debug(
          2,
          "Found active region header @ 0x%08x but no stale region @ 0x%08x. Trying to recreate stale region.",
          addr0, addr1);
    ...
    else
    {
      ...
      debug(1, "Found stale-region header @ 0x%08x, but no active region.",
            addr0);
      return SYSPARAM_ERR_CORRUPT;
    }
  }
  ...
  if (header0.flags_size & REGION_FLAG_ACTIVE)
  {
    ...
    debug(3,
          "Active region @ 0x%08x (0x%04x).  Stale region @ 0x%08x (0x%04x).",
          addr0, header0.flags_size, addr1, header1.flags_size);
  ...
  else
  {
    ...
    debug(3,
          "Active region @ 0x%08x (0x%04x).  Stale region @ 0x%08x (0x%04x).",
          addr1, header1.flags_size, addr0, header0.flags_size);
  }

  // Find the actual end
  _sysparam_info.end_addr = _sysparam_info.cur_base
      + _sysparam_info.region_size;
  _sysparam_info.force_compact = false;
  _init_context(&ctx);
...
sysparam_status_t sysparam_create_area(uint32_t base_addr, uint16_t num_sectors,
                                       bool force)
{
  size_t region_size;
  sysparam_status_t status;
  ...
  if (!force)
  {
    ...
    for (addr = base_addr; addr < base_addr + region_size * 2; addr +=
    SCAN_BUFFER_SIZE)
    {
      debug(3, "read %d words @ 0x%08x", SCAN_BUFFER_SIZE, addr);
      CHECK_FLASH_OP(
          spiflash_read(addr, (uint8_t*)buffer, SCAN_BUFFER_SIZE * 4));
  ...
  if (_sysparam_info.cur_base == base_addr || _sysparam_info.alt_base
      == base_addr)
  {
...
sysparam_status_t sysparam_get_data(const char *key, uint8_t **destptr,
                                    size_t *actual_length, bool *is_binary)
{
  struct sysparam_context ctx;
  sysparam_status_t status;
...
sysparam_status_t sysparam_get_data_static(const char *key, uint8_t *dest,
                                           size_t dest_size,
...
sysparam_status_t sysparam_get_bool(const char *key, bool *result)
{
  ...
  do
  {
    ...
    if (!strcasecmp(buf, "y") || !strcasecmp(buf, "yes")
        || !strcasecmp(buf, "t") || !strcasecmp(buf, "true")
        || !strcmp(buf, "1"))
    {
      *result = true;
      break;
    }
    if (!strcasecmp(buf, "n") || !strcasecmp(buf, "no") || !strcasecmp(buf, "f")
        || !strcasecmp(buf, "false") || !strcmp(buf, "0"))
    {
      *result = false;
      break;
    }
...
sysparam_status_t sysparam_set_data(const char *key, const uint8_t *value,
                                    size_t value_len, bool is_binary)
{
  struct sysparam_context ctx;
  ...
  do
  {
    ...
    if (value_len)
    {
      if (old_value_addr)
      {
        if ((ctx.entry.idflags & ENTRY_FLAG_BINARY) == binary_flag && ctx.entry
            .len
                                                                      == value_len)
        {
      ...
      // space.
      free_space = _sysparam_info.cur_base + _sysparam_info.region_size
          - _sysparam_info.end_addr;
      ...
      if (needed_space > free_space)
      {
        ...
        // this value)
        debug(1, "region full (need %d of %d remaining)", needed_space,
              free_space);
      ...
      // Write new value
      status = _write_entry(write_ctx.addr,
                            key_id | ENTRY_FLAG_VALUE | binary_flag, value,
                            value_len);
      if (status < 0)
        break;
    ...
    debug(
        1,
        "New addr is 0x%08x (%d bytes remaining)",
        _sysparam_info.end_addr,
        _sysparam_info.cur_base + _sysparam_info.region_size - _sysparam_info
            .end_addr);
  } while (false);
...
sysparam_status_t sysparam_iter_next(sysparam_iter_t *iter)
{
  ...
  while (true)
  {
    ...
    {
      iter->binary = true;
      debug(2, "iter_next: (0x%08x) '%s' = (0x%08x) <binary-data> (%d)",
            ctx->addr, iter->key, value_ctx.addr, iter->value_len);
    }
    else
    {
      iter->binary = false;
      debug(2, "iter_next: (0x%08x) '%s' = (0x%08x) '%s' (%d)", ctx->addr,
            iter->key, value_ctx.addr, iter->value, iter->value_len);
    }
