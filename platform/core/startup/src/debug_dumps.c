...
typedef void __attribute__((noreturn)) (*fatal_exception_handler_fn)(
    uint32_t *sp, bool registers_saved_on_stack);
static void __attribute__((noreturn)) standard_fatal_exception_handler_inner(
    uint32_t *sp, bool registers_saved_on_stack);
static void __attribute__((noreturn)) second_fatal_exception_handler_inner(
    uint32_t *sp, bool registers_saved_on_stack);
static void __attribute__((noinline)) __attribute__((noreturn)) abort_handler_inner(
    uint32_t *caller, uint32_t *sp);

static IRAM_DATA fatal_exception_handler_fn fatal_exception_handler_inner =
 ...
 * safely.
 */
void IRAM __attribute__((noreturn)) fatal_exception_handler(
    uint32_t *sp, bool registers_saved_on_stack)
{
...
void dump_stack(uint32_t *sp)
{
  ...
  for (uint32_t *p = sp; p < sp + 32; p += 4)
  {
    ...
    if (p[0] == 0xa5a5a5a5 && p[1] == 0xa5a5a5a5 && p[2] == 0xa5a5a5a5
        && p[3] == 0xa5a5a5a5)
    {
...
/* Main part of fatal exception handler, is run from flash to save
 some IRAM.
 */
static void standard_fatal_exception_handler_inner(
    uint32_t *sp, bool registers_saved_on_stack)
{
 ...
 */
static void second_fatal_exception_handler_inner(uint32_t *sp,
                                                 bool registers_saved_on_stack)
{
  dump_excinfo();
  printf(
...
void dump_heapinfo(void)
{
   ...
   */
  printf(
      "arena (total_size) %d fordblks (free_size) %d uordblocks (used_size) %d\n",
