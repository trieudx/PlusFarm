...
sdk_flashchip_t sdk_flashchip = { 0x001640ef,      // device_id
...
int IRAM sdk_SPIReadModeCnfig(uint32_t mode)
{
  ...
  SPI(0).CTRL0 &= ~(SPI_CTRL0_FASTRD_MODE | SPI_CTRL0_DOUT_MODE
                    | SPI_CTRL0_QOUT_MODE | SPI_CTRL0_DIO_MODE
                    | SPI_CTRL0_QIO_MODE);
  if (mode == 0)
  {
...
sdk_SpiFlashOpResult IRAM sdk_SPIWrite(uint32_t des_addr, uint32_t *src_addr,
                                       uint32_t size)
{
  ...
  {
    first_page_portion = sdk_flashchip.page_size
        - (des_addr % sdk_flashchip.page_size);
  ...
  if (full_pages)
  {
    ...
    {
      if (SPI_page_program(&sdk_flashchip, des_addr + pos, src_addr + (pos / 4),
                           sdk_flashchip.page_size))
      {
        return SPI_FLASH_RESULT_ERR;
      }
...
sdk_SpiFlashOpResult IRAM sdk_SPIRead(uint32_t src_addr, uint32_t *des_addr,
                                      uint32_t size)
{
...
sdk_SpiFlashOpResult IRAM sdk_spi_flash_write(uint32_t des_addr,
...
sdk_SpiFlashOpResult IRAM sdk_spi_flash_read(uint32_t src_addr,
                                             uint32_t *des_addr, uint32_t size)
{
  sdk_SpiFlashOpResult result;
