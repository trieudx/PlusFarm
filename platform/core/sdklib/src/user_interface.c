...
#include "sdk/esp_sta.h"
#include "sdk/esp_softap.h"
#include "sdk/esp_misc.h"
#include "sdk/osapi.h"
...
_Static_assert(sizeof(struct param_dir_st) == 28,
               "param_dir_st is the wrong size");
...
void IRAM sdk_system_restart_in_nmi(void)
{
  ...
  Cache_Read_Disable();
  DPORT.SPI_CACHE_RAM &=
      ~(DPORT_SPI_CACHE_RAM_BANK0 | DPORT_SPI_CACHE_RAM_BANK1);
  // This calls directly to 0x40000080, the "reset" exception vector address.
  _rom_reset_vector();
}

bool IRAM sdk_system_rtc_mem_write(uint32_t des_addr, void *src_addr,
                                   uint16_t save_size)
{
...
bool IRAM sdk_system_rtc_mem_read(uint32_t src_addr, void *des_addr,
                                  uint16_t save_size)
{
...
uint32_t sdk_system_get_userbin_addr(void)
{
  ...
  if (!(sdk_g_ic.s.boot_info >> 7))
  {
    ...
    {
      addr = sdk_g_ic.s.user0_addr[0] | (sdk_g_ic.s.user0_addr[1] << 8)
             | (sdk_g_ic.s.user0_addr[2] << 16);
    }
  }
  else
  {
...
bool sdk_system_upgrade_userbin_set(uint8_t userbin)
{
  ...
  else
  {
    ...
    sdk_g_ic.s._unknown1d8 = (sdk_g_ic.s._unknown1d8 & userbin_mask)
        | userbin_val;
    return true;
  }
}
...
void sdk_system_deep_sleep(uint32_t time_in_us)
{
  ...
  sdk_ets_timer_setfn(&deep_sleep_timer, _deep_sleep_phase2,
                      (void *)time_in_us);
...
void sdk_system_station_got_ip_set(struct ip_addr *ip, struct ip_addr *mask,
                                   struct ip_addr *gw)
{
  ...
  printf("ip:%d.%d.%d.%d,mask:%d.%d.%d.%d,gw:%d.%d.%d.%d", ip_bytes[0],
         ip_bytes[1], ip_bytes[2], ip_bytes[3], mask_bytes[0], mask_bytes[1],
         mask_bytes[2], mask_bytes[3], gw_bytes[0], gw_bytes[1], gw_bytes[2],
         gw_bytes[3]);
  printf("\n");
...
void sdk_system_print_meminfo(void)
{
  ...
  printf("%s: 0x%x ~ 0x%x, len: %d\n", "bss   ", _bss_start, _bss_end,
         _bss_end - _bss_start);
  printf("%s: 0x%x ~ 0x%x, len: %d\n", "heap  ", _heap_start, 0x3fffc000,
         0x3fffc000 - _heap_start);
}
...
bool sdk_wifi_station_dhcpc_start(void)
{
  ...
  if (netif && sdk_dhcpc_flag == DHCP_STOPPED)
  {
    ...
    netif_set_addr(netif, &sdk_info.sta_ipaddr, &sdk_info.sta_netmask,
                   &sdk_info.sta_gw);
    if (dhcp_start(netif))
    {
      return false;
