...
static const unsigned char secret_key[16] =
    { 0xf4, 0x82, 0xc6, 0x70, 0x3c, 0xc7, 0x61, 0x0a, 0xb9, 0xa0, 0xb8, 0xe9,
      0x87, 0xb8, 0xc1, 0x72, };
...
static int example(void)
{
  ...
  ret = mbedtls_ctr_drbg_seed(&drbg, mbedtls_entropy_func, &entropy, secret_key,
                              sizeof(secret_key));
  if (ret != 0)
  {
  ...
  ret = mbedtls_cipher_setup(
      &ctx, mbedtls_cipher_info_from_type(MBEDTLS_CIPHER_AES_128_CCM));
  if (ret != 0)
  {
  ...
  ret = mbedtls_cipher_setkey(&ctx, secret_key, 8 * sizeof secret_key,
                              MBEDTLS_ENCRYPT);
  if (ret != 0)
  {
  ...
  ret = mbedtls_cipher_auth_encrypt(&ctx, ciphertext, nonce_len,
                                    ...
                                    sizeof message, ciphertext + nonce_len,
                                    &ciphertext_len,
  ...
  ret = mbedtls_cipher_auth_decrypt(&ctx, ciphertext, nonce_len,
                                    (const unsigned char *)metadata,
