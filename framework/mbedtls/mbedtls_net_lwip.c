                     ...
                     const char *port, int proto)
{
  ...
  struct sockaddr_in sockaddr_ipaddr_any = { .sin_len =
  ...
  struct addrinfo all_interfaces_addr = {
      .ai_family = AF_INET, .ai_socktype = hints.ai_socktype, .ai_protocol =
          hints.ai_protocol,
      .ai_addrlen = sizeof(struct sockaddr_in), .ai_addr =
          (struct sockaddr *)&sockaddr_ipaddr_any, };
                       ...
                       size_t buf_size, size_t *ip_len)
{
  ...
  if (type != SOCK_STREAM)
  {
    ...
    if (getsockname(client_ctx->fd, (struct sockaddr *)&local_addr, &n) != 0 || (bind_ctx
