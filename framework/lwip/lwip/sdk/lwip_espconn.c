...
#include "lwip/app/lwip_espconn_tcp.h"
#include "lwip/app/lwip_espconn_udp.h"
#include "lwip/app/lwip_espconn.h"
...
espconn_copy_partial(struct espconn *pesp_dest, struct espconn *pesp_source)
{
  ...
  if (pesp_source->type == ESPCONN_TCP)
  {
    ...
    os_memcpy(pesp_dest->proto.tcp->local_ip, pesp_source->proto.tcp->local_ip,
              4);
    pesp_dest->proto.tcp->connect_callback = pesp_source->proto.tcp
        ->connect_callback;
    pesp_dest->proto.tcp->reconnect_callback = pesp_source->proto.tcp
        ->reconnect_callback;
    pesp_dest->proto.tcp->disconnect_callback = pesp_source->proto.tcp
        ->disconnect_callback;
  }
  else
  {
                                               ...
                                               espconn_msg **pnode)
{
  ...
  while (plist != NULL )
  {
    ...
    else
    {
      ...
      if ((ip_list.addr == ip_remot.addr) && (pespconn->proto.tcp->remote_port
...
espconn_connect(struct espconn *espconn)
{
  ...
  for (plist = plink_active; plist != NULL ; plist = plist->pnext)
  {
    ...
    {
      if (espconn->proto.tcp->local_port == plist->pespconn->proto.tcp
          ->local_port)
      {
...
espconn_create(struct espconn *espconn)
{
  ...
  for (plist = plink_active; plist != NULL ; plist = plist->pnext)
  {
    ...
    {
      if (espconn->proto.udp->local_port == plist->pespconn->proto.udp
          ->local_port)
      {
        return ESPCONN_ISCONN;
      }
                            ...
                            uint8 typeflags)
{
  ...
  switch (pespconn->type)
  {
    case ESPCONN_TCP:
      while (plist != NULL )
      {
        if ((plist->pespconn->type == ESPCONN_TCP) && (plist->preverse
            == pespconn))
        {
          switch (typeflags)
          {
            case ESPCONN_SSL:
              if (plist->pssl != NULL)
              {
                ...
                premot[pespconn->link_cnt].remote_port = plist->pcommon
                    .remote_port;
