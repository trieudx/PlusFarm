...
#include "osapi.h"
#include "lwip/app/lwip_dhcpserver.h"
...
static void ICACHE_FLASH_ATTR send_offer(struct dhcps_msg *m)
{
  ...
  SendOffer_err_t = udp_sendto(pcb_dhcps, p, &broadcast_dhcps,
                               DHCPS_CLIENT_PORT);
#if DHCPS_DEBUG
...
{
  if (os_memcmp((char *)m->options, (char *)magic_cookie, sizeof(magic_cookie)) == 0)
  {
#if DHCPS_DEBUG
...
//	            if(os_memcmp((char *)old_xid, (char *)m->xid, sizeof(m->xid)) != 0){
...

    {
      ...
      POOL_START: client_address.addr = client_address_plus.addr;
//							addr_tmp.addr =  htonl(client_address_plus.addr);
//							addr_tmp.addr++;
//							client_address_plus.addr = htonl(addr_tmp.addr);
      for (pback_node = plist; pback_node != NULL ;
...
//									os_printf("the ip addr has been request\n");
      ...
      POOL_CHECK: if ((client_address_plus.addr > dhcps_lease.end_ip)
          || (ip_addr_isany(&client_address)))
      {
...
bool ICACHE_FLASH_ATTR wifi_softap_set_dhcps_lease(struct dhcps_lease *please)
{
  ...
  softap_ip >>= 8;
  if ((please->start_ip >> 8 != softap_ip) || (please->end_ip >> 8 != softap_ip))
  {
    return false;
  }
